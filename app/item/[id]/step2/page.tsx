 ...
 * - + 신규 플로우 만들기
 * - 플로우 클릭 → /item/[id]/step2/[flowId] (상세 편집)
 */
...
export default function Step2HomePage() {
  ...
  function handleNew() {
    ...
    // ✅ 만들자마자 상세 편집으로 이동 (UX 좋음)
    router.push(`/item/${id}/step2/${flowId}`);
  }
  ...
  return (
    <main style={{ padding: 24, maxWidth: 1100, margin: "0 auto", fontFamily: "system-ui" }}>
      ...
      <div style={{ marginTop: 14, border: "1px solid #eee", borderRadius: 12, overflow: "hidden", background: "#fff" }}>
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((f) => (
            <div
              key={f.id}
              onClick={() => router.push(`/item/${id}/step2/${f.id}`)}
              style={{
                display: "grid",
      ...
      <div style={{ marginTop: 14 }}>
        <Link href={`/item/${id}`}>← Step1 상세로 돌아가기</Link>
      </div>
    </main>
  );
}